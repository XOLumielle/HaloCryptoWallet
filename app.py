...
def webhook():
    ...
    for tx in txs:
        ...
        balance_usd = f"${{:.2f}}".format(balance_sol * sol_price) if balance_sol and sol_price else "N/A"

        # Token info via Dexscreener
        token_name, symbol, token_price, liquidity = get_token_info(token_address)
        price_info = "${{:.4f}}".format(token_price) if token_price else "?"
        liquidity_info = "${{:.0f}}".format(liquidity) if liquidity else "?"

        msg = (
            "*Smart Whale Alert!*\n"
            f"*Wallet:* {label}\n"
            f"*Tx Type:* {tx_type}\n"
            f"*Spent:* {sol_spent:.4f} SOL (${{:.2f}})\n".format(sol_spent * sol_price) +
...
def home():
    return "HaloWebhook with Dexscreener is live!", 200

if __name__ == "__main__":
    app.run(debug=True, host="0.0.0.0", port=8080)from flask import Flask, request
import requests
import os
import json
